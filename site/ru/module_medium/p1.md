...
:::info FSM State Controller
Если ваш модуль использует Finite State Machine (определив свойство `state_machine`), вы можете получать экземпляр контроллера непосредственно в своих обработчиках. Фреймворк автоматически определит подпись функции и передаст контроллер.

Чтобы использовать его, определите свой обработчик так, чтобы он принимал контроллер в качестве дополнительного аргумента:
```python
# FSM-контроллер передается в качестве аргумента 'sm'
# Подпись с клиентом: (self, client, update, sm)
async def on_stateful_command(self, client: Client, message: Message, sm: MyStateMachine):
    current_state = await sm.get_state()
    await sm.set_state(self.state_machine.some_other_state)
    await message.reply(f"Your state was {current_state}, now it is some_other_state.")

# Подпись без клиента: (self, update, sm)
@command("state")
async def on_state_short(self, message: Message, sm: MyStateMachine):
    await message.reply(f"Your current state is: {await sm.get_state()}")
```
:::
