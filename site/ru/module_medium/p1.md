...
1. **__init__.py**: Этот файл заставляет Python рассматривать каталог как пакет. Он должен импортировать класс вашего главного модуля (см. шаг 2).
...
Определите асинхронный метод в классе `MyModule`. Метод должен принимать `self`, а также опционально `client` (экземпляр Pyrogram Client), `message` (или другой тип обновления) и `sm_controller` (для FSM).
...
- `@command(names, filters=None, fsm_state=None):` Регистрирует обработчик команд.
  ...
  - `fsm_state:` Optional State or list of States for FSM control (see [🚦 Finite State Machine (FSM)](https://pbmodular.github.io/wiki/module_medium/p9)).
...
Вы можете легко добавить Rate Limit в любой обработчик с помощью декоратора `@rate_limit`. Он должен быть размещен *перед* декоратором обработчика.

- `@rate_limit(rate=1, per=5, scope="user", queue=False):`
  - `rate`: Количество разрешенных вызовов в течение временного окна.
  - `per`: Временной интервал в секундах.
  - `scope`: Элемент, который необходимо ограничить (`"user"`, `"chat"`, or `"global"`).
  - `queue`: Если `True`, то вместо того, чтобы отбрасывать запросы, превышающие лимит, он будет ставить их в очередь и выполнять после требуемой задержки.

```python
from base.module import command, rate_limit

@rate_limit(rate=1, per=10, scope="user") # 1 запрос на пользователя каждые 10 секунд
@command("slow")
async def slow_command(self, message: Message):
    """Команда с rate limit."""
    await message.reply("Эту команду можно использовать один раз в 10 секунд.")
