...
If your module uses a Finite State Machine (by defining the `state_machine` property), you can receive the state controller instance directly in your handlers. The framework automatically detects the function's signature and passes the controller.

To use it, define your handler to accept the controller as an extra argument:
```python
# FSM controller is passed as the 'sm' argument
# Signature with client: (self, client, update, sm)
...
# Signature without client: (self, update, sm)
@command("state")
